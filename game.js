...
class Game {
  constructor() {
    this.pieces = [
      new Piece("rookb1", "A8", "black", "rook"),
    ...
    if (position.length > 2) {
      ...
      if (del != null) {
        ...
        for (let i = 1; i <= posibilities.length; i++) {
          if (
            posibilities[i] == del.currentLocation &&
            this.isEmpty(posibilities[i - 1])
          ) {
            return 2;
          }
        }
    ...
    if (piece.color === this.turn) {
      ...
      switch (piece.type) {
        case "pawn":
          if (piece.color === "black") {
            if (currRow == newRow) {
              ...
              if (
                piece.currentLocation === piece.initialLocation &&
                newCol + 2 >= currCol &&
                this.isEmpty(pos)
              ) {
                return 1;
              ...
              (currRow + 1 == newRow && newCol + 1 == currCol) ||
              (currRow - 1 == newRow && newCol == currCol - 1)
            ) {
        ...
        case "rook":
          if (newRow == currRow) {
            if (newCol < currCol) {
              var down = this.freeDownward(piece, position);
              return down;
            } //Se puede mover para abajo
            else if (newCol > currCol) {
              var up = this.freeUpward(piece, position);
              return up;
            } //Se puede mover hacia arriba
          } else if (newCol == currCol) {
            if (newRow > currRow) {
              var right = this.freeRightward(piece, position);
              return right;
            } //Se puede mover hacia la derecha
            else if (newRow < currRow) {
              var left = this.freeLeftward(piece, position);
              return left;
        ...
        case "bishop":
          ...
          if (newCol > currCol && newRow > currRow) {
            posibilities = this.bishopPosibilities(piece, position, 1);
          } else if (newCol > currCol && newRow < currRow) {
            posibilities = this.bishopPosibilities(piece, position, 2);
          } else {
            if (newCol < currCol && newRow < currRow) {
              posibilities = this.bishopPosibilities(piece, position, 3);
            } else if (newCol < currCol && newRow > currRow) {
              posibilities = this.bishopPosibilities(piece, position, 4);
            }
          }
